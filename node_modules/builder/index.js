...
var Builder = function(el){
	this.form = el;
	this.blockWrapper = el.querySelector('.builder--block-types ul');
	this.template = el.querySelector('.builder--template');
	this.blocks = this.blockWrapper.querySelectorAll('li');
	...
	for (i = 0; i < len; i++){
		this.blocks[i].setAttribute('draggable', true);
	}
...
Builder.prototype.addBlock = function(zoneName, type, style, data){
	if (!blocks[type]){
		return console.error('undefined block type', type);
	}
